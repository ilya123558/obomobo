...
import style from './CatalogTags.module.scss'
import 'swiper/css';
...
export const CatalogTags = () => {
  ...
  return (
    <div className='sm:h-[64px] h-[50px] flex items-center'>
          ...
          >
            {list.map((title, index) => (
              <SwiperSlide key={index} className='max-w-fit !flex !items-center'>
                <button className="md:p-[8px_15px] border-[1px] border-[#ECEEF0] p-[5px_10px] select-none rounded-[6px] bg-white text-black md:text-[14px] text-[11px] cursor-pointer will-change-transform transition-all active:scale-95">{title}</button>
              </SwiperSlide>
            ))}
          </Swiper>
          {showNavigation && (
