...
import { DateRangeSelector } from '@/features/date-range-selector/DateRangeSelector';
import { HeaderProfile } from '@/features/header-profile/HeaderProfile';
import { SelectPrice } from '@/features/select-price/SelectPrice';
import { SelectRole } from '@/features/select-role/SelectRole';
import { Theme } from '@/features/theme/Theme';
import { Search } from '@/shared/ui/search/Search';
import Link from 'next/link';
import React from 'react';

export const DeviceManagerHeader = () => {
  return (
    <header className={`sm:h-[64px] h-[45px] border-b-[2px] border-[#F5F6F7] flex items-center p-[0px_14px] justify-between `}>
      <div className="flex items-center">
        <div className="flex items-center">
          ...
          <div className={`2xl:block hidden font-semibold text-[18px] text-green ml-[15px]`}>Back stage</div>
        </div>
      ...
      <div className="lg:flex hidden items-center gap-[10px] ml-[10px]">
        <SelectRole />
        <SelectPrice />
      </div>
      <Search type='device-manager' />
      <div className="flex items-center">
        <Theme />
        <ActivityFeed />
        <HeaderProfile />
