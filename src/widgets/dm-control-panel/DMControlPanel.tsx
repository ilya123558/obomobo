...
import style from './DMControlPanel.module.scss';
import 'swiper/css';

export const DMControlPanel = () => {
  const prevRef = useRef(null);
  const nextRef = useRef(null);
  const [showNavigation, setShowNavigation] = useState(false); // Состояние для показа/скрытия кнопок
  const swiperRef = useRef<any>(null); // Реф для получения экземпляра swiper
  ...
  useEffect(() => {
    const handleResize = () => {
      if (swiperRef.current) {
        ...
        const visibleSlides = Math.floor(swiperRef.current.el.offsetWidth / slideWidth) + 1;

        setShowNavigation(visibleSlides < list.length);
      }
    };
  ...
  return (
    <div className='bg-[#F5F6F7] md:h-[64px] h-[40px] flex items-center'>
      <div className="w-screen">
        <div className={`${showNavigation ? '2xl:mx-[4%] xl:mx-[5%] sm:mx-[6%] mx-[10%]': 'mx-[1%]'} relative`}>
          <Swiper
            onSwiper={(swiper) => { swiperRef.current = swiper; }}
            spaceBetween={10}
            slidesPerView={'auto'}
            navigation={showNavigation ? { prevEl: '.job-slider_prev_btn', nextEl: '.job-slider_next_btn' } : false}
            modules={[Navigation]}
          >
            {list.map((title, index) => (
              <SwiperSlide key={index} className='max-w-fit !flex !items-center'>
                <button className="md:p-[8px_15px] border-[1px] border-[#ECEEF0] p-[5px_10px] select-none rounded-[6px] bg-white text-[#758195] md:text-[14px] text-[11px] cursor-pointer will-change-transform transition-all active:scale-95">{title}</button>
              </SwiperSlide>
            ))}
          </Swiper>
          {showNavigation && (
